...
def plot_candlestick(df: pd.DataFrame, symbol: str) -> None:
    ...
    try:
        ...
        if isinstance(df_sorted.columns, pd.MultiIndex):
            df_sorted.columns = df_sorted.columns.droplevel(1)
...
def display_daily_stock_data(df: pd.DataFrame, symbol: str) -> None:
    ...
    try:
        ...
        if isinstance(display_df.columns, pd.MultiIndex):
            display_df.columns = display_df.columns.droplevel(1)
        ...
        if 'Date' in display_df.columns:
            ...
            display_df.rename(columns={"": "Date"}, inplace=True)

...
Compare the performance of portfolio components with a view to better optimization
""")
...
#Create the annulalized covariance matrix
...
#Calculate the annual portforlio return
...
#Get the disccrete allocation of each share per stock
...
#Merge the two dataframes and then drop the duplicate colunm 'stocks' from old tables
...
st.write('Invested ${:.2f}'.format(total_invested) + ', leaving a cash balance of ${:.2f}'.format(leftover_corrected))
st.write('Portfolio stocks changed by {:.2%}'.format(total_percent_gain) + ', for a total gain of ${:.2f}'.format(total_gain))
st.write('Total Portfolio value is now ${:.2f}'.format(final_portfolio_value))
